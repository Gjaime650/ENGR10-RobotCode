...
#pragma config(Motor,  port3,           armMotor,      tmotorServoStandard, openLoop)
...
task main(){
	...
	steer_sensitivity = 5;//used in move
	forward_speed = 50;//forward speed , used in move
	slow_speed = 35;//slow speed , used in move
	spin_speed = 30;//spin speed (for searching mode),used in move
	SensorValue[digital10] = freq;// turn to 1KHz(red beacon)

	setServo(armMotor,-120);
	wait(1);
	...
	setServo(armMotor, 100);
	int belowAmbientCount = 0;
	while(state == 2){
		ReadPD();
		if(PD_sum <= ambient_level){
			belowAmbientCount++;
			if(belowAmbientCount >= 1){

				setServo(armMotor,-120);
				wait(1);

				startMotor(leftMotor, forward_speed);
				startMotor(rightMotor, -forward_speed);
				wait(1.5);
	...
	//turn off the green and capture it
	setServo(armMotor, 100);
	belowAmbientCount = 0;
	while(state == 4){
		ReadPD();
	...
	while(state == 5){
		...
		if(distance > 366){ //furthest distance between boundary wall robot is 12ft which equals ~366 cm
			startMotor(leftMotor, forward_speed);
			startMotor(rightMotor, forward_speed);
			wait(3.0);
			stopMotor(leftMotor);
			stopMotor(rightMotor);
			state = 6;
			} else { //keeps spinning untill distance detected is greater than 366cm
			startMotor(leftMotor, spin_speed);
			startMotor(rightMotor, -spin_speed);
			wait(0.2);
		}
	}
