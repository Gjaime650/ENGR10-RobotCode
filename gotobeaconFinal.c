...
#pragma config(Sensor, dgtl1,  DgtlBumper,     sensorTouch)
...
#pragma config(Motor,  port2,           leftMotor,     tmotorNone, openLoop)
...
task main(){
	...
	while(state == 1){
		...
		Find_max();
		Move();
		lmtSwitch = SensorValue[DgtlBumper];
		if(lmtSwitch == 1){
			stopMotor(port1);
			stopMotor(port3);
			state = 2;
